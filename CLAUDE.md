...
`advanceTo` is a fixed-step integrator, not a closed-form formula: it runs whole `STEP_MS` steps and advances `lastTick` by exactly the time it consumed, carrying the sub-step remainder to the next call. That is what makes the result depend on elapsed time alone rather than on how it was split into calls, which matters because population, food, and jobs are a coupled loop. Offline progress is the same code path — a `now` far ahead of `lastTick` just runs more steps, up to `MAX_OFFLINE_MS` worth. Past that the absence is **capped, not coarsened**: the steps stop and `lastTick` jumps to `now`, so the excess is forfeited rather than deferred. Deferring it would be a throttle instead of a cap — the next call would find the same time still owed and simulate another day of it, and another. The cost of a cap is that split-independence only holds up to it: a week away in one call is a day of progress, where the same week checked in on daily is seven. All sim rules go in `simulateStep`; the loop around it only decides how many steps to run.
...
- **Surfaces are plain, and the header is opaque on purpose:** cards, the drawer and the progress tracks take Mantine's own surface colours; the only app-level tokens left are `--app-header-bg` and `--app-progress-track` in `styles/app.css`, plus the light-scheme `--mantine-color-body` that moves the page off the card colour. The header is the one rule with a reason behind it: it is opaque and `position: sticky`, because Safari 26 tints the iOS status bar from whatever fixed or sticky element sits at the top of the viewport and reads its `background-color` — a flat colour gets extended upward, while a translucent or `backdrop-filter`ed one gets a glass treatment that blurs the top of the app. There was a fixed photograph behind everything until the blur it provoked outstayed its welcome; the image, the scrim, the translucent `--app-surface-*` tokens and the `Card`/`ActionIcon` overrides that read them all went at once, and `git log -- src/assets` is where to start if it comes back.

- **Viewport units on an installed iOS app:** `.app-shell` sets the document's height, and it takes `100svh` in a browser tab but `100vh` under `@media (display-mode: standalone)`. That is not belt-and-braces: iOS misreports the dynamic units (`svh`, `dvh`) on a cold start, and a cold start is every launch there, since a backgrounded PWA is killed rather than suspended. A shell that opens short ends the document short, and the strip of page background below it is visible on any screen whose content does not overshoot the shortfall — which is how this was found, with the pā screen clean and About and the absence report showing a band. Resist folding the two rules into one: `max()` of them hands a tab the taller figure and the rubber-band `svh` is there to avoid. The app is drawn edge-to-edge on iOS whether or not it asks to be, so the safe-area insets on the header, the page's bottom spacer and the nav drawer are all load-bearing rather than leftovers.
...
- **Build stamp:** the drawer's footer names the commit the running bundle was built from, because `package.json`'s `version` is a placeholder for an app that is never published. `vite.config.ts` bakes the short sha in as a `__GIT_SHA__` define (falling back to `GITHUB_SHA`, then `'unknown'`, so a missing `.git` can never fail a build); `src/buildInfo.ts` is the only module that reads it. Vitest does not read `vite.config.ts`, so `vitest.config.ts` carries the same define with a stub value — adding another build-time constant means adding it in both places.
